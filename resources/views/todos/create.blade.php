...
@section('content')
    <div class="sm:flex sm:justify-center min-h-screen">
        <div class="container mx-auto text-center">
            <div class="text-3xl font-semibold font-roboto my-4">Create Todo</div>
            <br>
            <div class="max-w-md mx-auto md:max-w-lg px-2 md:px-0">
                <form action="{{ route('todos.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="text-left my-4">
                        ...
                        <input type="text" name="detail" class="w-full bg-slate-100 rounded-md p-2 border @error('detail') border-red-400 @enderror" value="{{ isset($oldData['detail']) ? $oldData['detail'] : '' }}">
                        @error('detail')
                            <div class="text-sm text-red-500">*{{ $message }}</div>
                        @enderror
                    </div>
